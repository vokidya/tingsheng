...
module.exports = {
  ...
  resolve: {
    alias: { // 配置目录别名
      ...
      vue: path.join(root, 'node_modules/vue/dist')
    },
    extensions: ['.js', '.vue'], // 引用js和vue文件可以省略后缀名
    modules: [path.join(root, 'node_modules')] // 找不到的模块会尝试在这个数组的目录里面再寻找
  },
  resolveLoader: {
    modules: [path.join(root, 'node_modules')] // 找不到的loader模块会尝试在这个数组的目录里面再寻找
  },
  module: { // 配置loader
    rules: [
